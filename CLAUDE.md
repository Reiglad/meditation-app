...
- **Service Workerのキャッシュ更新**：`index.html` / `css` / `js`（`config.js`の音声設定変更を含む）/ `manifest.json` / `icons` のいずれかを更新したら、**必ず `sw.js` の `CACHE_NAME` の数字をインクリメントする**こと。Service Workerはファイル内容がバイト単位で前回と同一だと更新を検知せず、古いキャッシュを使い続けてしまう。CACHE_NAMEを変えるとsw.js自体のバイト内容が変わり、確実に新バージョンとして再キャッシュされる。
- **アプリを開いたときの自動更新**：`sw.js`は`skipWaiting()` / `clients.claim()`で新バージョンを即座に有効化する設定にしており、`index.html`側でも`registration.update()`をアプリ起動時（`load`）とバックグラウンド復帰時（`visibilitychange`でvisibleになった時）に明示的に呼んで更新チェックを行う。新しいService Workerが制御を引き継いだ（`controllerchange`）タイミングで自動的に`window.location.reload()`する（瞑想・NSDR実施中の場合はセッションが失われないようリロードを保留し、次にアプリを開いたときに反映する）。iOS Safariの制約上、バックグラウンドで勝手に最新化されることはなく、あくまで「アプリを開いたら自動で最新版になる」までが実現できる範囲。それでも反映されない場合はSafariで一度サイトを開き直す（ホーム画面から削除して再追加、またはSafariの「Webサイトデータを消去」）ことで解消できる。GitHub Pages側のHTTPキャッシュが残ることもあり、その場合はハードリロード（PC）や上記のサイトデータ消去（iPhone）で解消する。
